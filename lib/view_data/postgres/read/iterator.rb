    ...
    class Read
      class Iterator
        ...
        def get_batch
          ...
          batch, last_position = get.(position, include: :last_position)
        ...
        def query_depleted?
          if not batch_initialized?
            logger.debug { "Query is not depleted (Batch Length: (batch is nil), Batch Size: #{batch_size})" }
            return false
          end

          if batch.length < batch_size
            logger.debug { "Query is depleted (Batch Length: #{batch.length}, Batch Size: #{batch_size})" }
            return true
          end

          logger.debug { "Query is not depleted (Batch Length: #{batch.length}, Batch Size: #{batch_size})" }
          false
        end
